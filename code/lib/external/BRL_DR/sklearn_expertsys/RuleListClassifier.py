...
import numpy as np
import pandas as pd
from lib.external.BRL_DR.sklearn_expertsys.LethamBRL.BRL_code import *
from lib.external.BRL_DR.sklearn_expertsys.Discretization.MDLP import *
import numbers
...
class RuleListClassifier(BaseEstimator):
    ...
    def fit(self, X, y, feature_labels=[], undiscretized_features=[]):
        ...
        # Do MCMC
        res, Rhat = run_bdl_multichain_serial(self.max_iter, self.thinning, self.alpha, self.listlengthprior, self.listwidthprior, Xtrain, Ytrain, nruleslen,
                                              lhs_len, self.maxcardinality, permsdic, self.burnin, self.n_chains, [None] * self.n_chains, verbose=self.verbose)
        ...
        self.d_star = get_point_estimate(permsdic, lhs_len, Xtrain, Ytrain, self.alpha, nruleslen, self.maxcardinality,
                                         self.listlengthprior, self.listwidthprior, verbose=self.verbose)  # get the point estimate
        ...
        if self.d_star:
            ...
            for i, j in enumerate(self.d_star):
                ...
                s += condition + " probability of " + self.class1label + ": " + \
                    str(np.round(self.theta[i] * 100, decimals)) + "% (" + str(np.round(self.ci_theta[i][0] * 100, decimals)) + "%-" + str(np.round(self.ci_theta[i][1] * 100, decimals)) + "%)\n"
    ...
    def predict_proba(self, X):
        ...
        if self.discretizer != None:
            ...
            D = self._prepend_feature_labels(np.array(X)[:, :-1])
        else:
            D = X
